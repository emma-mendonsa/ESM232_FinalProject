...
whitePine_PopDynamics_model = function(clim_df,clim_scen_name,impact_type,scenario_parms,pine_parms,beetle_parms,beetle_K){
  ...
  output_df = data.frame(year = clim_df$year, 
                        ...
                        btl_rmax = rep(NA,years), # Will be filled in with initial_r_max * btl_r_coef. Used to caculate the beetle population size in each time step
  ...
  return(output_df) 
    ...
    if(impact_type == "beetles_and_climate"){
