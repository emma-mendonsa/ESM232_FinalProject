...
combined = data.frame(year = 2010:2100, 
                      ...
                      pine0 = rep(NA,91), # Number of pines in stage 0
                      pine1 = rep(NA,91), # Number of pines in stage 1
...
combined[1,24] = 256876 # Initial carbon = post-fire cabon b/c no fire occurred yet
...
################################################# Pines ######################
  ...
  #pine_biomass = pine[[3]] # Pull out biomass across stages from Pine Model

  #fire_prob = combined[i,21] # Fire probability in current time step
  #fire_sev = combined[i,22] # Fire severity in current time step
  #forest_carbon = combined[i,20] #Forest carbon in current time step
  
  #fire = burnt_biomass_model(pine_biomass, fire_prob, fire_sev, forest_carbon)
  
  #post_fire_carb = fire[[2]]
  #combined$post_fire_carbon[i] = post_fire_carb #Pull out Post-Fire Carbon and place into main output dataframe
}

climate = combined$forest_carbon # Putting output of carbon into it's own dataframe to plot with others
climate_fire = combined$post_fire_carbon #to plot with others
```
