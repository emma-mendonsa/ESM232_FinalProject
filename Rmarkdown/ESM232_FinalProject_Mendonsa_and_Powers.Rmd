...
```{r}
 diffs = data.frame(year = climate_only$year,climate_only$total_pine,beetles_only$total_pine,beetles_and_climate$total_pine) %>% 
  melt(id.vars = "year")

ggplot(diffs,aes(x = year, y = value, color = variable))+
  geom_line()
```
