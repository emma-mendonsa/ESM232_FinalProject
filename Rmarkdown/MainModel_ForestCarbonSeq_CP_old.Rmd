...
# set up parmaters
parms = list(p0=btl_pop_0, r=r_0, K=K_0)
result = ode(parms$p0, beetles$time, beetle_pop, parms)
...
for(i in 2:nrow(beetles)){
  ...
  parms = list(p0=beetles$pop[i-1], r=beetles$r_max[i], K=K_0)
  result = ode(parms$p0, beetles$time, beetle_pop, parms)
...
for(i in 2:years){
  ...
  combined$btl_pop[i] = beetle_pop(N=combined$btl_pop[i-1],
                                   ...
                                   K = K_0*log(sum(combined[i-1,16:18])))
